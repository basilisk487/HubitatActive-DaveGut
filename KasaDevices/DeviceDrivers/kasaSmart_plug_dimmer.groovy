...
library (
    name: "lib_tpLink_CapDimmer",
    namespace: "davegut",
    author: "Compied by Dave Gutheinz",
    description: "Hubitat Capability Switch Level and Change Level Methods for TPLink SMART devices.",
    category: "utilities",
    documentationLink: ""
)
...
def setGradualOnOff() {
    ...
    def cmdResp = syncPassthrough(createMultiCmd(requests))
    def gradOnOffData = cmdResp.result.responses.find { it.method == "get_on_off_gradually_info" }
    def newGradualOnOff = gradOnOffData.result.enable
    device.updateSetting("gradualOnOff",[type:"bool", value: newGradualOnOff])
    return newGradualOnOff
}
...
def setLevel(level, transTime=null) {
    ...
    asyncPassthrough(createMultiCmd(requests), "setLevel", "deviceParse")
}
